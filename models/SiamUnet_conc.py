...
class SiamUnet_conc(nn.Module):
    ...
    def __init__(self, input_nbr, label_nbr):
        ...
        self.conv12d3d = nn.ConvTranspose2d(48, 16, kernel_size=3, padding=1)
        self.bn12d3d = nn.BatchNorm2d(16)
        self.do12d3d = nn.Dropout2d(p=0.2)
        self.conv11d3d = nn.ConvTranspose2d(16, 1, kernel_size=3, padding=1)
    ...
    def forward(self, x1, x2):
        ...
        x1d3d = self.upconv1(x21d)
        pad13d = ReplicationPad2d((0, x12_1.size(3) - x1d3d.size(3), 0, x12_1.size(2) - x1d3d.size(2)))
        x1d3d = torch.cat((pad13d(x1d3d), x12_1, x12_2), 1)
        x12d3d = self.do12d3d(F.relu(self.bn12d3d(self.conv12d3d(x1d3d))))
        x11d3d = self.conv11d3d(x12d3d)

        #Softmax layer is embedded in the loss layer
        out2D = self.sm(x11d)
        out3D = self._3dact(x11d3d)
        output = []
        output.append(out2D)
