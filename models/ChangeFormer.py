# taken from https://github.com/wgcban/ChangeFormer
...
class DecoderTransformer_v3(nn.Module):
    ...
    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True, 
        ...
        #Final predction head 3D
        self.convd2x3d    = UpsampleConvLayer(self.embedding_dim, self.embedding_dim, kernel_size=4, stride=2)
        self.dense_2x3d   = nn.Sequential( ResidualBlock(self.embedding_dim))
        self.convd1x3d    = UpsampleConvLayer(self.embedding_dim, self.embedding_dim, kernel_size=4, stride=2)
        self.dense_1x3d   = nn.Sequential( ResidualBlock(self.embedding_dim))
        self.elevation_change = ConvLayer(self.embedding_dim, 1, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, inputs1, inputs2):
        ...
        #Upsampling x2 (x1/2 scale)
        x3d = self.convd2x3d(_c)
        #Residual block
        x3d = self.dense_2x3d(x3d)
        #Upsampling x2 (x1 scale)
        x3d = self.convd1x3d(x3d)
        #Residual block
        x3d = self.dense_1x3d(x3d)
        #Final prediction3D
        ec = self.elevation_change(x3d)
        ...
        outputs.append(self.active3d(ec))
...
class ChangeFormerV6(nn.Module):
    ...
    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False, embed_dim=256):
        ...
        self.Tenc_x2    = EncoderTransformer_v3(img_size=256, patch_size = 7, in_chans=input_nc, num_classes=output_nc, embed_dims=self.embed_dims,
                 num_heads = [1, 2, 2, 4], mlp_ratios=[4, 4, 4, 4], qkv_bias=True, qk_scale=None, drop_rate=self.drop_rate,
                 attn_drop_rate = self.attn_drop, drop_path_rate=self.drop_path_rate, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 depths=self.depths, sr_ratios=[8, 4, 2, 1])
    ...
    def forward(self, t1, t2):
        ...
        [fx1, fx2] = [self.Tenc_x2(t1), self.Tenc_x2(t2)]
        ...
        # exit()
        out2d=cp[-2]
        # out2d=torch.tensor(out2d)
        out3d=cp[-1]
        # out3d=torch.tensor(out3d)
        return out2d, out3d
